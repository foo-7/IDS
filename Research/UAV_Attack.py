"""
To be converted into Jupyter Notebook for Google Colab

** For now, files are found locally.
"""
import os
...
from sklearn.multiclass import OneVsRestClassifier # ADDED TO USE LOCALLY AND NOT GOOGLE COLAB

jamming = 'data/jamming-merged-gps-only.csv'
spoofing = 'data/spoofing-merged-gps-only.csv'
df_jamming = pd.read_csv(jamming)
df_spoofing = pd.read_csv(spoofing)
df_jamming['Source'] = jamming
df_spoofing['Source'] = spoofing

final_df = pd.concat([df_spoofing, df_jamming], ignore_index=True)
del df_jamming, df_spoofing
gc.collect()

final_df['label'], mapping = pd.Series(final_df['label']).factorize()
print('[FILE READ]: All files have been combined and labels have been refactorized')

# Dropping these features since they act like target leaks
final_df = final_df.drop(columns=[
    'jamming_indicator', # direct flag
    'noise_per_ms', # spikes during jamming
    'fix_type', # gps fix type drops during spoofing or jamming
    'satellites_used' # drop in satellites is a dead giveaway
])

# Having issues with splits at >= 3
tscs = TimeSeriesSplit(n_splits=2)
final_df = final_df.sort_values('timestamp').reset_index(drop=True)
X = final_df.drop(columns=['label', 'Source'])
y = final_df['label']

# Group is still required for sequence sequence integrity
# so that our method create_sequences function will not make weird windows
# like when spoofing ends and then the jamming begins, which would be weird
groups = final_df['Source']

# Same create_sequence method found in ISOT-Drone
def create_sequences(df, window_size):
    ...
    for src in sources:
        ...
        if group.empty or len(group) < window_size: continue
    ...
    if not sequence: return np.array([], dtype=np.float32), np.array([], dtype=np.int64)
    return np.array(sequence, dtype=np.float32), np.array(labels, dtype=np.int64)

model_names = ['CNN', 'RF-SVC', 'RF', 'SVM', 'XGBOOST', 'MLP']
metric_names = ['accuracy', 'macro-f1-score', 'per-class-f1-score', 'recall', 'precision']
MODELS = {
...
DRIVE_ROOT = 'progress/UAV_Attack/'
progressFile = os.path.join(DRIVE_ROOT, 'experiment_progress.json')
if os.path.exists(progressFile):
    with open(progressFile, 'r') as f:
        MODELS = json.load(f)
    print("Resuming from saved progress (SAFETY MEASURE FROM CRASHED KERNEL)")


for fold, (train_index, test_index) in enumerate(tscs.split(X, y, groups)):
    """
    There are some del lines in case that we need to free up memory from the RAM/VRAM
    """
    if len(MODELS['CNN']['accuracy']) > fold:
        print(f'Skipping Fold {fold+1} - Already completed.')
        continue

    print(f'\n{"="*30}[PROCESS]: STARTING FOLD {fold + 1}/2{"="*30}')
    ...
    # CHECKING IF THERE IS A PROBELM WITH GROUP-KFOLD
    unique_train_classes = np.unique(y_train_xgb)
    if len(unique_train_classes) < 2:
        print(f"[SKIP WARNING]: Fold {fold + 1} only contains one class ({unique_train_classes}). Skipping models that require multiple classes.")
        continue
    ...
    SVC = OneVsRestClassifier(SVC) # DELETE WHEN USED IN GOOGLE COLAB
    #SVC.fit(RF_train_features_gpu, y_train_gpu_xgb)
    SVC.fit(cp.asnumpy(RF_train_features_gpu), cp.asnumpy(y_train_gpu_xgb)) # Remove this when using Google Colab
    ...
    hybrid_preds = SVC.predict(cp.asnumpy(RF_test_features)) # Remove when using Google Colab
    ...
    SVC.fit(cp.asnumpy(X_train_gpu), cp.asnumpy(y_train_gpu_xgb)) # Remove this when using Google Colab
    
    #pred = SVC.predict(X_test_gpu_filtered)
    pred = SVC.predict(cp.asnumpy(X_test_gpu_filtered)) # Remove when using Google Colab
    ...
    del XGBOOST, pred, acc, y_pred_cpu, macrof1, recall, prec
    gc.collect()
    ...
    """
    # CNN
    fold_path = f'best_weights/CNN_UAV_Attack_fold_{fold+1}.pth'
    ...
    fold_path = f'best_weights/MLP_UAV_Attack_fold_{fold+1}.pth'
    ...
    with open(progressFile, 'w') as f:
        json.dump(MODELS, f)
    print(f"[SAVE]: Fold {fold+1} results synced to Drive")
